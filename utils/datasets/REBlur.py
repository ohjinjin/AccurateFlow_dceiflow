...
class REBlur(dataset.Dataset):
    ...
    def fetch_valids(self):
        ...
        event_filenames = sorted(
            glob(os.path.join(events_root, "*-event.h5")))
...
#         final_indices = []
        for i in range(len(image1_filenames)):
            im1_base_fileid = '-'.join((os.path.basename(
                image1_filenames[i])).split('-')[:-1])
            im2_base_fileid = '-'.join((os.path.basename(
                image2_filenames[i])).split('-')[:-1])
            flow_f_base_fileid = '-'.join((os.path.basename(
                flow01_filenames[i])).split('-')[:-1])
            flow_b_base_fileid = '-'.join((os.path.basename(
                flow10_filenames[i])).split('-')[:-1])
            event_base_fileid = '-'.join((os.path.basename(
                event_filenames[i])).split('-')[:-1])
...
#                 final_indices.append(i)
        ...
        self.image1_filenames = image1_filenames#[image1_filenames[i] for i in final_indices]
        self.image2_filenames = image2_filenames#[image2_filenames[i] for i in final_indices]
        self.flow01_filenames = flow01_filenames#[flow01_filenames[i] for i in final_indices]
        self.flow10_filenames = flow10_filenames#[flow10_filenames[i] for i in final_indices]
        self.event_filenames = event_filenames#[event_filenames[i] for i in final_indices]
...
#         print("im1_nparray.shape", im1_nparray.shape)
#         print("events_nparray.shape", events_nparray.shape)

        return im1_nparray, im2_nparray, flow01_nparray, flow10_nparray, events_nparray
#         return im1_nparray, events_nparray, flow01_nparray
    ...
    def __getitem__(self, index):
        ...
        im1_filename = self.image1_filenames[index]
        basename = '-'.join((os.path.basename(im1_filename)).split('-')[:-1])
#         print("chchchchjinjin::" , im1_filename)
#         print("chchchchjinjin'::" , basename)
#         print('-----------------------------------')

        im1_nparray, im2_nparray, flow01_nparray, flow10_nparray, events_nparray = \
            self.load_data_by_index(index)
#         im1_nparray, events_nparray, flow01_nparray = \
#             self.load_data_by_index(index)
        ######################################
...
#         height, width = red.shape
        ...
        dst_path = "/research/DCEIFlow/result_reblur_test/"
...
#         height, width = im1_nparray.shape[:2]
        ...
        if self.args.isbi:
            event_x = np.flip(events_nparray[:, 0].astype(np.int64), axis=0)
            event_y = np.flip(events_nparray[:, 1].astype(np.int64), axis=0)
            event_pols = np.flip(-1 * events_nparray[:, 3].astype(np.int64), axis=0)
